...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={clsx(
          'fixed inset-y-0 left-0 z-50 lg:static bg-dark-800 border-r border-dark-600/50 transition-all duration-300 flex flex-col',
          sidebarOpen ? 'w-64 translate-x-0' : 'w-20 -translate-x-full lg:translate-x-0 lg:w-20'
        )}
      >
        {/* Logo */}
        <div className="flex items-center justify-between px-4 py-5 border-b border-dark-600/50">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-accent rounded-lg flex items-center justify-center flex-shrink-0">
              <Beer size={16} className="text-dark-900" />
            </div>
            {(sidebarOpen || !sidebarOpen) && (
              <div className={clsx("overflow-hidden transition-all duration-300", sidebarOpen ? "w-auto opacity-100" : "w-0 lg:w-0 opacity-0")}>
          ...
          {sidebarOpen && (
            <button onClick={toggleSidebar} className="lg:hidden p-1.5 rounded-lg text-dark-200 hover:text-white hover:bg-dark-700">
              <X size={18} />
            </button>
          )}
        </div>
            ...
            return (
              ...
              >
                ...
                <span className={clsx(
                  ...
                  sidebarOpen ? "opacity-100 w-auto" : "lg:opacity-0 lg:w-0"
                )}>
                  {item.label}
                </span>
                {active && sidebarOpen && (
        ...
        <button
          onClick={toggleSidebar}
          className="hidden lg:flex items-center justify-center p-2 border-t border-dark-600/50 text-dark-200 hover:text-white hover:bg-dark-700 transition-all"
        >
